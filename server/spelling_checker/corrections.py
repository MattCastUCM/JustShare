...
class SpellCorrector:
	...
	def get_ranked_candidates(self, sentence: list[str], position: int, max_candidates: int = 200, unigram_weight: float = 0.5, lower_sentence: Optional[list[str]] = None):
		if lower_sentence is None:
			lower_sentence = [w.lower() for w in sentence]
		...
		left_context = lower_sentence[max(0, position - (fwd_n - 1)):position]
		missing_left = (fwd_n - 1) - len(left_context)
		left_context = [self.forward_lm.bos_token] * missing_left + left_context

		bwd_n = self.backward_lm.order
		right_context = list(reversed(sentence[position + 1:position + bwd_n]))
	...
	def correct_word(self, sentence: list[str], position: int, max_candidates: int = 200, unigram_weight: float = 0.5, enhanced: bool = False, lower_sentence: Optional[list[str]] = None) -> str:
		if lower_sentence is None:
			lower_sentence = [w.lower() for w in sentence]
		...
		if enhanced:
			...
			lower_sentence[position] = collapsed
