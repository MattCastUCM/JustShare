...
class IdfdWord2Vec(Word2Vec):
	...
	def _idf_weights(self, tokens: list[str]):
		weights = [
			self.idf_dict.get(token, np.log(self.n_docs))
			for token in tokens
		]
...
class POSdWord2Vec(Word2Vec):
	...
	def _pos_weights(self, pos_tags: list[str]):
		weights = [
			POS_WEIGHTS.get(pos, DEFAULT_POS_WEIGHT)
			for pos in pos_tags
		]
		return np.array(weights)
...
class IdfPosWord2Vec(Word2Vec):
	def __init__(self, wv: KeyedVectors, tokenizer_fn: Callable[[str], list[str]], pos_fn: Callable[[str], list[str]]):
		super().__init__(SearchMethod.WORD2VEC_IDF_POS, wv, tokenizer_fn)
		self.pos_tagger = pos_fn
	...
	def _transform(self, texts: list[str]):
		...
		for tokens, pos_tags in zip(tokenized_docs, pos_docs):
			...
			idf_weights = self._idf_weights(tokens)
			pos_weights = self._pos_weights(pos_tags)

			weights = idf_weights * pos_weights
